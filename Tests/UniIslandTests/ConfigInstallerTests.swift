...
final class ConfigInstallerTests: XCTestCase {
    ...
    func testFlatExternalHooksPassEventFlagToBridge() throws {
        ...
        XCTAssertTrue(command.contains("uniisland-bridge --source cursor"))
    ...
    func testTraecliJsonExternalHooksPassEventFlagToBridge() throws {
        ...
        let command = try XCTUnwrap(entries.first?["command"] as? String)
        XCTAssertTrue(command.contains("uniisland-bridge --source traecli"))
    ...
    func testRemoveKimiHooksPreservesNonUniIslandBlocks() {
        ...
        [[hooks]]
        event = "Stop"
        command = "/Users/test/.uniisland/uniisland-bridge --source kimi"
        timeout = 5
        ...
        XCTAssertFalse(cleaned.contains("uniisland-bridge"))
    ...
    func testInstallKimiHooksIntegration() throws {
        ...
        XCTAssertTrue(installed.contains("uniisland-bridge --source kimi"))
        ...
        XCTAssertFalse(uninstalled.contains("uniisland-bridge"), "UniIsland hooks should be removed after uninstall")
    }
    ...
    func testMergeCocoHooksAppendsHooksSectionWhenMissing() {
        ...
        XCTAssertTrue(cmd?.contains("uniisland-bridge --source traecli") ?? false)
...
hooks:
  - type: command
    ...
    matchers:
      - event: stop
  - type: command
    command: '\(NSHomeDirectory())/.uniisland/uniisland-bridge --source traecli'
    timeout: '86400s'
    matchers:
      - event: session_start
      - event: session_end
...
"""
        ...
        XCTAssertEqual(commands.filter { $0.contains("uniisland-bridge") && $0.contains("--source traecli") }.count, 1)
        XCTAssertEqual(hooks.count, 2)
    }

    func testMergeTraecliHooksRemovesQuotedBridgeCommandToAvoidDuplicates() {
        let bridge = "\(NSHomeDirectory())/.uniisland/uniisland-bridge"
        let original = """
hooks:
  - type: command
...
"""
        ...
        let hooks = try! yamlHooks(merged)
        let commands = hooks.compactMap { $0["command"] as? String }
        XCTAssertEqual(commands.filter { $0.contains("uniisland-bridge") }.count, 1)
...
hooks:
  ...
  # UNIISLAND_MANAGED_TRAECLI_HOOK_BEGIN
  - type: command
    command: '\(NSHomeDirectory())/.uniisland/uniisland-bridge --source traecli'
    matchers:
      - event: stop
  # UNIISLAND_MANAGED_TRAECLI_HOOK_END
  # trailing comment should also be removed
"""

        let cleaned = ConfigInstaller.removeManagedTraecliHooks(from: original)

        XCTAssertFalse(cleaned.contains("uniisland-bridge --source traecli"))
        XCTAssertFalse(cleaned.contains("UNIISLAND_MANAGED_TRAECLI_HOOK"))
...
"""
    ...
    func testRemoteInstallerConfigureScriptInstallsOpencodePlugin() {
        ...
        XCTAssertTrue(script.contains("uniisland-opencode-remote.js"))
    ...
    func testRemoteOpencodePluginCarriesRemoteHostIdentity() throws {
        ...
        let source = """
        const SOCKET_PATH = process.env.UNIISLAND_SOCKET_PATH || "/tmp/uniisland.sock";
        const REMOTE_HOST_ID = process.env.UNIISLAND_REMOTE_HOST_ID || "";
        const REMOTE_HOST_NAME = process.env.UNIISLAND_REMOTE_HOST_NAME || "";
        """

        let plugin = RemoteInstaller.remoteOpencodePluginForInstall(source: source, host: host)

        XCTAssertTrue(plugin.contains(#"const SOCKET_PATH = "/tmp/uniisland.sock";"#))
    ...
    func testRemoteInstallerConfigureScriptInjectsPerUserSocketPath() {
        ...
        // socket path so different OS users don't collide on /tmp/uniisland.sock.
        let host = RemoteHost(id: "host-1", name: "devbox", host: "example.com")

        let script = RemoteInstaller.configureRemoteHooksScript(host: host, remoteSocketPath: "/tmp/uniisland-1000.sock")

        XCTAssertTrue(script.contains(#"socket_path = "/tmp/uniisland-1000.sock""#))
        XCTAssertTrue(script.contains("UNIISLAND_SOCKET_PATH={socket_path}"))
        XCTAssertFalse(script.contains("UNIISLAND_SOCKET_PATH=/tmp/uniisland.sock"))
    }
    ...
    func testRemoteInstallerConfigureScriptFallsBackToLegacySocketPath() {
        ...
        XCTAssertTrue(script.contains(#"socket_path = "/tmp/uniisland.sock""#))
    }

    func testRemoteOpencodePluginInjectsPerUserSocketPath() {
        // #193
        let host = RemoteHost(id: "host-1", name: "devbox", host: "example.com")
        let source = #"const SOCKET_PATH = process.env.UNIISLAND_SOCKET_PATH || "/tmp/uniisland.sock";"#

        let plugin = RemoteInstaller.remoteOpencodePluginForInstall(source: source, host: host, remoteSocketPath: "/tmp/uniisland-1000.sock")

        XCTAssertTrue(plugin.contains(#"const SOCKET_PATH = "/tmp/uniisland-1000.sock";"#))
    }
    ...
    func testMergeOpencodePluginRefCreatesMinimalConfigWhenFileAbsent() throws {
        let merged = try XCTUnwrap(
            ConfigInstaller.mergeOpencodePluginRef(
                ...
                pluginRef: "file:///tmp/uniisland.js",
                identifier: "uniisland"
            )
        )
        ...
        XCTAssertEqual(json["plugin"] as? [String], ["file:///tmp/uniisland.js"])
    }
    ...
    func testMergeOpencodePluginRefPreservesUnrelatedKeysAndOtherPlugins() throws {
        ...
        XCTAssertTrue(plugins.contains("file:///tmp/uniisland.js"))
    }
    ...
    func testMergeOpencodePluginRefDeduplicatesOurOwnRefs() throws {
        ...
        let merged = try XCTUnwrap(
            ConfigInstaller.mergeOpencodePluginRef(
                originalContents: original,
                pluginRef: "file:///new/uniisland.js",
                identifier: "uniisland"
            )
        )
        let json = try XCTUnwrap(try JSONSerialization.jsonObject(with: Data(merged.utf8)) as? [String: Any])
        let plugins = try XCTUnwrap(json["plugin"] as? [String])
        XCTAssertEqual(plugins.filter { $0.contains("uniisland") }.count, 1)
        XCTAssertFalse(plugins.contains { $0.contains("vibe-island") })
        XCTAssertTrue(plugins.contains("file:///user/other.js"))
        XCTAssertTrue(plugins.contains("file:///new/uniisland.js"))
    }
    ...
    func testRemoveOpencodePluginRefKeepsUserKeysAndOtherPlugins() throws {
        ...
        {
          "model": "sonnet",
          "plugin": ["file:///tmp/uniisland.js", "file:///user/other.js"]
        }
        """
    ...
    func testMergeOpencodePluginRefPreservesJSONCCommentsAndKeyOrder() throws {
        ...
        XCTAssertTrue(merged.contains("file:///tmp/uniisland.js"))
    ...
    func testRemoveOpencodePluginRefPreservesOriginalFormatting() throws {
        ...
        XCTAssertFalse(cleaned.contains("file:///tmp/uniisland.js"))
    ...
    func testInstallPiExtensionWritesBundledExtensionWhenPiAgentDirExists() throws {
        ...
        let piExtensionPath = piExtensionDir.appendingPathComponent("uniisland.ts")
        ...
        XCTAssertTrue(contents.contains("UniIsland pi extension"))
    ...
    func testUninstallPiExtensionOnlyRemovesUniIslandExtension() throws {
        let fm = FileManager.default
        let tempDir = fm.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        let piExtensionPath = tempDir.appendingPathComponent("uniisland.ts")
        let userExtensionPath = tempDir.appendingPathComponent("user.ts")
        try fm.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: tempDir) }
        try "// UniIsland pi extension\n// version: v1\n".write(to: piExtensionPath, atomically: true, encoding: .utf8)
    ...
    func testInstallOmpExtensionWritesBundledExtensionWhenOmpAgentDirExists() throws {
        ...
        let ompExtensionPath = ompExtensionDir.appendingPathComponent("uniisland.ts")
    ...
    func testUninstallOmpExtensionPreservesUserFileAtSamePath() throws {
        let fm = FileManager.default
        let tempDir = fm.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        let ompExtensionPath = tempDir.appendingPathComponent("uniisland.ts")
        try fm.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? fm.removeItem(at: tempDir) }
