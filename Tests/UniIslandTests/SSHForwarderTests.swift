...
final class SSHForwarderTests: XCTestCase {
    ...
    func testCleanupArgumentsBasic() {
        ...
        let args = SSHForwarder.cleanupArguments(host: host, remoteSocketPath: "/tmp/uniisland-501.sock")

        XCTAssertEqual(args, [
            "-o", "BatchMode=yes",
            "-o", "ConnectTimeout=5",
            "alice@192.168.1.10", "rm", "-f", "/tmp/uniisland-501.sock",
        ])
    }
    ...
    func testCleanupArgumentsIncludesPortAndIdentity() {
        ...
        let args = SSHForwarder.cleanupArguments(host: host, remoteSocketPath: "/tmp/uniisland-1000.sock")
        ...
        XCTAssertEqual(Array(suffix), ["bob@example.com", "rm", "-f", "/tmp/uniisland-1000.sock"])
    }
    ...
    func testCleanupArgumentsAlwaysIncludesRm() {
        ...
        let socketPath = "/tmp/uniisland-0.sock"
    ...
    func testCleanupArgumentsBatchModeAlwaysOn() {
        let host = RemoteHost(name: "test", host: "10.0.0.1", user: "root")
        let args = SSHForwarder.cleanupArguments(host: host, remoteSocketPath: "/tmp/uniisland-0.sock")
