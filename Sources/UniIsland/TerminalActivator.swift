    ...
    static func activate(session: SessionSnapshot, sessionId: String? = nil) {
        ...
        if session.source == "drops" {
            if let cwd = session.cwd {
                let dropsURL = URL(fileURLWithPath: cwd)
                NSWorkspace.shared.open(dropsURL)
            }
            return
        }
        ...
        tell application "System Events"
            tell process "\(escapeAppleScript(appName))"
                ...
                if bestWindow is not missing value then
                    -- AXRaise alone does NOT restore a window minimized to the Dock; clear
                    -- AXMinimized first so clicking the island reopens it (#1, also Claude/WeChat).
                    try
                        if value of attribute "AXMinimized" of bestWindow is true then
                            set value of attribute "AXMinimized" of bestWindow to false
                        end if
                    end try
                    perform action "AXRaise" of bestWindow
                else
                    -- No title match (e.g. WeChat, whose window title isn't the folder name):
                    -- restore the first minimized window so a minimized app still reopens.
                    repeat with w in windows
                        try
                            if value of attribute "AXMinimized" of w is true then
                                set value of attribute "AXMinimized" of w to false
