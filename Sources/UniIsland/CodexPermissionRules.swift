...
struct CodexPermissionRules {
    ...
    func persistAlwaysAllowRule(for event: HookEvent) -> Bool {
        ...
        let rulesPath = rulesDirectory + "/uniisland.rules"
    ...
    private static func ruleBlock(for pattern: [String]) -> String {
        """
        # Added by UniIsland when "Always Allow" is clicked for Codex.
        prefix_rule(
            \(patternLine(for: pattern)),
            decision = "allow",
            justification = "Allowed from UniIsland Always Allow",
        )
