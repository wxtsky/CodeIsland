...
private let log = Logger(subsystem: "com.uniisland", category: "AppState")
...
final class AppState {
    ...
    var pomodoroRemaining: TimeInterval = 25.0 * 60.0
    var pomodoroPaused = false
    var pomodoroTotalDuration: TimeInterval = 25.0 * 60.0
    ...
    var calendarJoinLink: String? = nil
    @ObservationIgnored private var calendarMonitorTimer: Timer?

    init() {
        loadDroppedFiles()
        startDropsCleanupTimer()
        startWidgetsTimers()
    }
    ...
    private var weChatFSEventStream: FSEventStreamRef?
    ...
    private var dismissedDiscoveredSessionIds: Set<String> = []
    ...
    func dismissSessionFromIsland(_ sessionId: String) {
        guard sessions[sessionId] != nil else { return }
        guard !permissionQueue.contains(where: { ($0.event.sessionId ?? "default") == sessionId }),
              !questionQueue.contains(where: { ($0.event.sessionId ?? "default") == sessionId }) else {
            return
        }

        dismissedDiscoveredSessionIds.insert(sessionId)
        sessions.removeValue(forKey: sessionId)
        stopMonitor(sessionId)
        detachTranscriptTailer(sessionId: sessionId)
        exitingSessions.removeValue(forKey: sessionId)
    ...
    func handleEvent(_ event: HookEvent) {
        ...
        dismissedDiscoveredSessionIds.remove(sessionId)
    ...
    func startSessionDiscovery() {
        ...
        // Start WeChat notification observer poller
        startWeChatPoller()
    }
    ...
    @ObservationIgnored
    private var weChatTimer: Timer?
    /// Last seen notification-DB signature (size+mtime) — re-read content only when it changes.
    @ObservationIgnored
    private var lastWeChatDBSignature = ""
    @ObservationIgnored
    private var lastWeChatUnreadKey = ""
    @ObservationIgnored
    private var lastDisplayedWeChatUnreadKey = ""
    @ObservationIgnored
    private var dismissedWeChatUnreadKey: String?
    @ObservationIgnored
    private var weChatBurstUntil: Date?
    @ObservationIgnored
    private var pendingImmediateWeChatPoll = false

    nonisolated static func weChatPollInterval(isBursting: Bool, burstUntil: Date?, now: Date = Date()) -> TimeInterval {
        if isBursting, let burstUntil, burstUntil > now {
            return 0.03
        }
        return 0.2
    }

    func startWeChatPoller() {
        guard weChatTimer == nil else { return }
        ...
        startWeChatNotificationDBWatcher()
        log.info("WeChat notification poller started")
    }

    private func scheduleNextWeChatPoll(after interval: TimeInterval? = nil) {
        weChatTimer?.invalidate()
        let nextInterval = interval ?? Self.weChatPollInterval(
            isBursting: weChatBurstUntil.map { $0 > Date() } ?? false,
            burstUntil: weChatBurstUntil
        )
        weChatTimer = Timer.scheduledTimer(withTimeInterval: nextInterval, repeats: false) { [weak self] _ in
            Task { @MainActor in
                self?.pollWeChatBadge()
            }
        }
    }

    private func enterWeChatBurstWindow(seconds: TimeInterval = 3.0) {
        let until = Date().addingTimeInterval(seconds)
        if weChatBurstUntil.map({ $0 < until }) ?? true {
            weChatBurstUntil = until
        }
    }

    private func requestImmediateWeChatPoll() {
        guard !pendingImmediateWeChatPoll else { return }
        pendingImmediateWeChatPoll = true
        enterWeChatBurstWindow()
        scheduleNextWeChatPoll(after: 0.01)
    }

    private func startWeChatNotificationDBWatcher() {
        guard weChatFSEventStream == nil else { return }
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        let dbDir = home + "/Library/Group Containers/group.com.apple.usernoted/db2"
        ...
        let stream = FSEventStreamCreate(
            ...
            { (_, info, _, _, _, _) in
                ...
                let appState = Unmanaged<AppState>.fromOpaque(info).takeUnretainedValue()
                Task { @MainActor in
                    appState.requestImmediateWeChatPoll()
                }
            },
            &context,
            [dbDir] as CFArray,
    ...
    private func pollWeChatBadge() {
        ...
        guard isWeChatRunning else {
            lastWeChatDBSignature = ""
            lastWeChatUnreadKey = ""
            lastDisplayedWeChatUnreadKey = ""
            hideWeChatSessionFromIsland(clearDismissal: true)
            weChatBurstUntil = nil
            scheduleNextWeChatPoll()
            return
        }

        let previousUnreadKey = lastWeChatUnreadKey
        let previousSignature = lastWeChatDBSignature
        Task.detached {
            let badge = Self.readWeChatBadgeViaAX()
            let signature = badge.isEmpty ? "" : Self.weChatDBSignature()
            let unreadKey = badge.isEmpty ? "" : "\(badge)|\(signature)"
            await MainActor.run { [weak self] in
                guard let self else { return }
                if !unreadKey.isEmpty && (unreadKey != previousUnreadKey || signature != previousSignature) {
                    self.enterWeChatBurstWindow()
                }
                self.lastWeChatDBSignature = signature
                self.lastWeChatUnreadKey = unreadKey
                self.handleWeChatBadgeResult(badge, unreadKey: unreadKey)
                self.scheduleNextWeChatPoll()
            }
        }
    }

    private func handleWeChatBadgeResult(_ badge: String, unreadKey: String) {
        ...
        if !badge.isEmpty {
            ...
            if sessions[wechatSessionId] == nil {
                sessions[wechatSessionId] = SessionSnapshot()
            ...
            // .waitingQuestion drives the "alert" mascot (jump + ! ) and the pulsing bell
            let isNewUnreadRecord = lastDisplayedWeChatUnreadKey != unreadKey
            if sessions[wechatSessionId]?.toolDescription != displayMsg || sessions[wechatSessionId]?.status != .waitingQuestion || isNewUnreadRecord {
                sessions[wechatSessionId]?.status = .waitingQuestion
                sessions[wechatSessionId]?.currentTool = "微信"
                sessions[wechatSessionId]?.toolDescription = displayMsg
                sessions[wechatSessionId]?.lastActivity = Date()
                lastDisplayedWeChatUnreadKey = unreadKey
    ...
    func dismissWeChatFromIsland() {
        ...
        dismissedWeChatUnreadKey = lastWeChatUnreadKey
        lastDisplayedWeChatUnreadKey = lastWeChatUnreadKey
        hideWeChatSessionFromIsland(clearDismissal: false)
    }

    private func hideWeChatSessionFromIsland(clearDismissal: Bool) {
        withAnimation(NotchAnimation.close) {
            if clearDismissal {
                dismissedWeChatUnreadKey = nil
            }
            ...
            let wechatSessionId = "wechat"
            let hadSession = sessions[wechatSessionId] != nil
            sessions.removeValue(forKey: wechatSessionId)
            completionQueue.removeAll { $0 == wechatSessionId }
            if activeSessionId == wechatSessionId {
                activeSessionId = mostActiveSessionId()
            }
            var didCollapseSurface = false
            if surface.sessionId == wechatSessionId {
                didCollapseSurface = true
                surface = .collapsed
            }
            guard hadSession || didCollapseSurface else { return }
            startRotationIfNeeded()
            refreshDerivedState()
        }
    }

    // Directory where dropped files are cached
    nonisolated static var dropsDirectory: URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let dir = appSupport.appendingPathComponent("UniIsland/Drops", isDirectory: true)
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir
    }

    /// Scan drops directory and reload files on main thread
    func loadDroppedFiles() {
        ...
        do {
            ...
            self.droppedFiles = urls.sorted { url1, url2 in
            ...
            log.error("Failed to load dropped files: \(error)")
        }
    }

    /// Handles files dropped via NSItemProvider
    func handleDroppedFiles(_ providers: [NSItemProvider]) {
        Task {
            ...
            let destDir = Self.dropsDirectory
            for url in loadedURLs {
                ...
                do {
                    ...
                    log.error("Failed to copy dropped file: \(error)")
                }
            }
            ...
            await MainActor.run {
                ...
                // Auto-expand the island to show dropped files
                if self.activeSessionId == nil || self.activeSessionId == "default" {
                    self.activeSessionId = "drops"
                }
                enqueueCompletion("drops")
            }
        }
    }

    /// Delete a dropped file
    func deleteDroppedFile(_ url: URL) {
        try? FileManager.default.removeItem(at: url)
        loadDroppedFiles()
    }

    /// AirDrop a file using native sharing service
    func shareDroppedFile(_ url: URL, sender: NSView) {
    ...
    private func updateDropsSession() {
        ...
        if !droppedFiles.isEmpty {
            if sessions[sessionId] == nil {
                sessions[sessionId] = SessionSnapshot()
                sessions[sessionId]?.source = "drops"
                sessions[sessionId]?.cwd = Self.dropsDirectory.path
                sessions[sessionId]?.termBundleId = "com.apple.Finder"
                sessions[sessionId]?.termApp = "Finder"
            }
            let count = droppedFiles.count
            sessions[sessionId]?.status = .waitingApproval // Pulse bell to notify user of drops
            sessions[sessionId]?.currentTool = "Drops"
            sessions[sessionId]?.toolDescription = "已存入 \(count) 个临时文件\n点击打开，Option+点击清除"
            sessions[sessionId]?.lastActivity = Date()
    ...
    @ObservationIgnored
    private var dropsCleanupTimer: Timer?

    private func startDropsCleanupTimer() {
        dropsCleanupTimer = Timer.scheduledTimer(withTimeInterval: 3600, repeats: true) { [weak self] _ in
            Task { @MainActor in
                self?.cleanupOldDroppedFiles()
            }
        }
        cleanupOldDroppedFiles() // Initial run
    }

    private func cleanupOldDroppedFiles() {
        let dir = Self.dropsDirectory
        ...
        do {
            ...
            for url in urls {
                if let date = (try? url.resourceValues(forKeys: [.creationDateKey]).creationDate),
                   now.timeIntervalSince(date) > 86400 { // 24 hours
                    try? fileManager.removeItem(at: url)
                    log.info("Auto-cleaned old dropped file: \(url.lastPathComponent)")
                }
            }
            loadDroppedFiles()
        } catch {
            log.error("Failed to clean dropped files: \(error)")
        }
    }

    /// Reads the WeChat Dock badge label via AXUIElement (no AppleScript/System Events needed).
    /// Returns the badge string (e.g. "3", "99+") or "" if none / inaccessible.
    private nonisolated static func readWeChatBadgeViaAX() -> String {
        guard let dock = NSRunningApplication.runningApplications(withBundleIdentifier: "com.apple.dock").first else {
            return ""
        }
        let dockElem = AXUIElementCreateApplication(dock.processIdentifier)

        // Dock top-level children → look for AXList
        var children: AnyObject?
        let childErr = AXUIElementCopyAttributeValue(dockElem, kAXChildrenAttribute as CFString, &children)
        guard childErr == .success, let childList = children as? [AXUIElement] else {
            // -25211 (kAXErrorAPIDisabled) means Accessibility permission is missing.
            if childErr == .apiDisabled {
                log.error("WeChat badge: Accessibility permission not granted (open System Settings → Privacy → Accessibility)")
            }
            return ""
        }

        // Find the first AXList (the app row in the Dock)
        var dockList: AXUIElement?
        for child in childList {
            var role: AnyObject?
            if AXUIElementCopyAttributeValue(child, kAXRoleAttribute as CFString, &role) == .success,
               (role as? String) == "AXList" {
                dockList = child
                break
            }
        }
        guard let dockList else { return "" }

        // Enumerate Dock items looking for WeChat
        var items: AnyObject?
        guard AXUIElementCopyAttributeValue(dockList, kAXChildrenAttribute as CFString, &items) == .success,
              let dockItems = items as? [AXUIElement] else {
            return ""
        }
    ...
    struct WeChatNotif: Equatable, Sendable { let sender: String; let body: String }

    /// Cheap change-signal for the notification DB: combined size+mtime of db and -wal.
    /// Lets the poller skip the (relatively expensive) copy+read unless something changed.
    private nonisolated static func weChatDBSignature() -> String {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        let base = home + "/Library/Group Containers/group.com.apple.usernoted/db2/db"
        let fm = FileManager.default
        var parts: [String] = []
        for suffix in ["", "-wal"] {
            if let attrs = try? fm.attributesOfItem(atPath: base + suffix) {
                let size = (attrs[.size] as? Int) ?? 0
                let mod = (attrs[.modificationDate] as? Date)?.timeIntervalSince1970 ?? 0
                parts.append("\(size):\(mod)")
            }
        }
        return parts.joined(separator: "|")
    }

    /// Reads the most recent WeChat notifications (newest first) from the macOS Notification
    /// Center database. Returns [] when unavailable (no Full Disk Access, no notifications).
    private nonisolated static func readWeChatNotifications(limit: Int) -> [WeChatNotif] {
        let home = FileManager.default.homeDirectoryForCurrentUser.path
        let srcBase = home + "/Library/Group Containers/group.com.apple.usernoted/db2/db"
        let fm = FileManager.default
        guard fm.fileExists(atPath: srcBase) else { return [] }

        // The notification DB runs in WAL mode: recent messages live in the -wal log and a
        // plain read-only open of the live main file misses them until macOS checkpoints
        // (which can lag for minutes → "shows old message, then the new one later").
        // Copy the main DB + -wal into a temp file and open the copy READ-WRITE so SQLite
        // runs WAL recovery and we always see the latest committed records.
        let tmpBase = NSTemporaryDirectory() + "ci_usernoted_\(UUID().uuidString).db"
        defer { for s in ["", "-wal", "-shm"] { try? fm.removeItem(atPath: tmpBase + s) } }
        do {
            try fm.copyItem(atPath: srcBase, toPath: tmpBase)
        } catch {
            return []
        }
        if fm.fileExists(atPath: srcBase + "-wal") {
            try? fm.copyItem(atPath: srcBase + "-wal", toPath: tmpBase + "-wal")
        }

        var db: OpaquePointer?
        guard sqlite3_open_v2(tmpBase, &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_NOMUTEX, nil) == SQLITE_OK,
              let db else {
        ...
        let sql = """
            ...
            WHERE app.identifier LIKE '%xinwechat%'
            ORDER BY record.rec_id DESC LIMIT \(max(1, limit))
        """
        guard let stmt = prepareSQLiteStatement(db: db, sql: sql) else { return [] }
        defer { sqlite3_finalize(stmt) }

        var result: [WeChatNotif] = []
        while sqlite3_step(stmt) == SQLITE_ROW {
            ...
            // The blob is a binary plist; message is under req.titl (sender) / req.body (text).
            guard let plist = try? PropertyListSerialization.propertyList(from: data, options: [], format: nil),
                  let dict = plist as? [String: Any],
                  let req = dict["req"] as? [String: Any] else { continue }
            ...
            result.append(WeChatNotif(sender: title, body: body))
        }
        return result
    }

    /// Builds the island text from recent notifications. Multiple senders are newline-separated;
    /// when crowded, show who sent messages rather than squeezing every message body in.
    nonisolated static func summarizeWeChatNotifications(_ notifs: [WeChatNotif], badge: String) -> String {
        let unreadNotifs = unreadWeChatNotificationsForDisplay(notifs, badge: badge)
        // Group by sender, newest first, keeping each sender's most recent message.
        var seen = Set<String>()
        var grouped: [WeChatNotif] = []
        for n in unreadNotifs {
            let key = n.sender.isEmpty ? n.body : n.sender
            if seen.insert(key).inserted { grouped.append(n) }
        }
        func line(_ n: WeChatNotif) -> String {
            if n.sender.isEmpty { return n.body }
            if n.body.isEmpty { return n.sender }
            return "\(n.sender): \(n.body)"
        }
        switch grouped.count {
        case 0:
            return badge.isEmpty ? "新消息" : "新消息 (\(badge))"
        case 1:
            return line(grouped[0])
        case 2:
            return grouped.prefix(2).map(line).joined(separator: "\n")
        default:
            let names = grouped.map { $0.sender.isEmpty ? "有人" : $0.sender }
            let shown = names.prefix(4).map { "\($0) 发来消息" }.joined(separator: "\n")
            if grouped.count <= 4 {
                return shown
            }
            return shown + "\n等 \(grouped.count) 人发来消息"
        }
    }

    nonisolated static func unreadWeChatNotificationsForDisplay(_ notifs: [WeChatNotif], badge: String) -> [WeChatNotif] {
        let trimmed = badge.trimmingCharacters(in: .whitespacesAndNewlines)
        if trimmed.hasSuffix("+"),
           let value = Int(trimmed.dropLast()) {
            return Array(notifs.prefix(max(1, value)))
        }
    ...
    private func integrateDiscovered(_ discovered: [DiscoveredSession]) {
        ...
        for info in discovered {
            if dismissedDiscoveredSessionIds.contains(info.sessionId) {
                continue
            }
    ...
    func stopSessionDiscovery() {
        ...
        if let stream = weChatFSEventStream {
            FSEventStreamStop(stream)
            FSEventStreamInvalidate(stream)
            FSEventStreamRelease(stream)
            weChatFSEventStream = nil
        }
    ...
    func togglePomodoro() {
        if pomodoroActive {
            pomodoroPaused.toggle()
        } else {
            let durationMinutes = UserDefaults.standard.double(forKey: SettingsKey.pomodoroDurationMinutes)
            let duration = (durationMinutes > 0 ? durationMinutes : 25.0) * 60.0
            pomodoroActive = true
            pomodoroPaused = false
            pomodoroRemaining = duration
            pomodoroTotalDuration = duration
            pomodoroLabel = "专注 Coding"
        }
    ...
    private func tickPomodoro() {
        guard pomodoroActive && !pomodoroPaused else { return }
        if pomodoroRemaining > 1 {
            ...
            notification.title = "番茄钟专注结束"
            notification.informativeText = "做得好！是时候休息 5 分钟了！"
    ...
    private func updateCalendarEvents() {
        ...
        case .notDetermined:
            eventStore.requestFullAccessToEvents { [weak self] granted, _ in
                if granted {
                    Task { @MainActor in
                        self?.fetchNextCalendarEvent()
                    }
                }
            }
        default:
            calendarEventTitle = "无日历访问权限"
            calendarEventCountdown = "请在系统隐私中开启"
            updateWidgetsSessions()
        }
    }
    ...
    private func fetchNextCalendarEvent() {
        ...
        if let nextEvent = events.first(where: { !$0.isAllDay && $0.startDate > start }) {
            let title = nextEvent.title ?? "无标题"
            let diff = Int(nextEvent.startDate.timeIntervalSince(start))
            let mins = diff / 60
            
            calendarEventTitle = title
            if mins < 60 {
                calendarEventCountdown = "\(mins) 分钟后开始 (\(formatTime(nextEvent.startDate)))"
            } else {
                let hours = mins / 60
                let remMins = mins % 60
                calendarEventCountdown = "\(hours)h \(remMins)m 后开始 (\(formatTime(nextEvent.startDate)))"
            }
            
            calendarJoinLink = extractMeetingLink(from: nextEvent)
        } else {
            calendarEventTitle = "无近期日程"
            calendarEventCountdown = "暂无日程"
            calendarJoinLink = nil
        }
        updateWidgetsSessions()
    }
    ...
    private func extractMeetingLink(from event: EKEvent) -> String? {
        let fields = [event.location, event.notes, event.url?.absoluteString].compactMap { $0 }
        let patterns = [
            "https://[^\\s]*zoom\\.us/j/[^\\s]*",
        ...
        for field in fields {
            for pattern in patterns {
                if let regex = try? NSRegularExpression(pattern: pattern, options: .caseInsensitive),
                   let match = regex.firstMatch(in: field, options: [], range: NSRange(location: 0, length: field.utf16.count)) {
                    if let range = Range(match.range, in: field) {
    ...
    func updateWidgetsSessions() {
        ...
        if isMediaEnabled && !mediaTrackName.isEmpty {
            if sessions[mediaId] == nil {
                var snap = SessionSnapshot()
                snap.source = "media"
                snap.cwd = "/"
                snap.currentTool = "媒体控制"
                sessions[mediaId] = snap
            }
            sessions[mediaId]?.status = isMediaPlaying ? .processing : .idle
            sessions[mediaId]?.toolDescription = isMediaPlaying ? "正在播放: \(mediaTrackName) - \(mediaArtistName)" : "已暂停"
            sessions[mediaId]?.lastActivity = Date()
        } else {
