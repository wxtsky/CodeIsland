...
struct DiagnosticsExporter {
    ...
    static func export() {
        ...
        panel.nameFieldStringValue = "UniIsland-Diagnostics-\(timestamp()).zip"
    ...
    private static func buildArchive(saveTo destination: URL) throws -> URL {
        ...
        let tmp = fm.temporaryDirectory.appendingPathComponent("UniIsland-Diag-\(UUID().uuidString)", isDirectory: true)
        let root = tmp.appendingPathComponent("UniIsland-Diagnostics-\(timestamp())", isDirectory: true)
        ...
        let configs: [(source: String, dest: String)] = [
            ...
            ("\(home)/.uniisland/sessions.json", "configs/persisted-sessions.json"),
        ]
        for item in configs {
        ...
        let logOutput = runCommand("/usr/bin/log", args: [
            ...
            "--last", "2h", "--predicate", "subsystem == \"com.uniisland\""
        ])
    ...
    private static func copyCrashReports(to dir: URL) {
        ...
        let recent = files
            .filter { $0.lastPathComponent.lowercased().contains("uniisland") }
            .sorted {
