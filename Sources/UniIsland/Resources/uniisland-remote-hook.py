...
VERSION = "0.1.2"
# Per-user socket path (#193): UniIsland injects UNIISLAND_SOCKET_PATH via the hook
# command, but fall back to a uid-scoped path so multiple users on a shared host never
# collide on a single /tmp/uniisland.sock.
SOCKET_PATH = os.environ.get("UNIISLAND_SOCKET_PATH") or f"/tmp/uniisland-{os.getuid()}.sock"
REMOTE_HOST_ID = os.environ.get("UNIISLAND_REMOTE_HOST_ID", "")
REMOTE_HOST_NAME = os.environ.get("UNIISLAND_REMOTE_HOST_NAME", "")
SOURCE = os.environ.get("UNIISLAND_SOURCE", "")
TIMEOUT_SECONDS = 300


def _normalize_event(name):
    """Best-effort normalization matching UniIslandCore.EventNormalizer."""
