// UniIsland pi extension
// version: v1

/**
 * @fileoverview UniIsland Integration Extension.
 *
 * Bridges the running pi session to the UniIsland macOS floating-window app
 * (https://github.com/wxtsky/UniIsland) by forwarding lifecycle events over
 * the Unix domain socket UniIsland listens on.
 *
 * Architecture:
 *   pi (this extension)  ──→  /tmp/uniisland-{uid}.sock  ──→  UniIsland.app
 *
 * The extension is a socket CLIENT — no server is started. If UniIsland is not
 ...
 *   sent as a blocking PermissionRequest via the uniisland-bridge binary. The
 *   extension waits for UniIsland's decision and returns allow/block accordingly.
 *   This replaces the built-in permission-gate.ts when UniIsland is active.
 *
 * Installation:
 *   Drop this file in ~/.pi/agent/extensions/ — it is auto-discovered.
 *
 * Requirements:
 *   - UniIsland.app running on the same machine
 */
...
/** Unix socket path UniIsland listens on (user-scoped). */
const userId = getuid?.() ?? 0;
const SOCKET_PATH = `/tmp/uniisland-${userId}.sock`;
 ...
 */
const BRIDGE_PATH = `${homedir()}/.uniisland/uniisland-bridge`;

/** Environment variable keys forwarded to UniIsland for terminal detection. */
const ENV_KEYS = [
  "TERM_PROGRAM",
  "ITERM_SESSION_ID",
  "TERM_SESSION_ID",
...
/**
 * Sends a JSON payload to the UniIsland socket (fire-and-forget).
 * Returns `false` silently when UniIsland is not running.
 *
...
/**
 * Sends a JSON payload via the bridge binary and waits for UniIsland's response.
...
/**
 * Builds the base fields required on every UniIsland event payload.
 *
...
export default function uniislandExtension(pi: ExtensionAPI) {
  /** TTY path detected once at startup. */
  const tty = detectTty();

  /**
   * Session IDs for which a blocking PermissionRequest is currently in flight.
   * Non-lifecycle events for these sessions are suppressed to prevent UniIsland's
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (
      ...
      if (behavior?.behavior === "deny") {
        return { block: true, reason: "Blocked by UniIsland" };
      }
