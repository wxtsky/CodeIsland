...
enum RemoteInstaller {
    ...
    /// OS users on a shared host don't collide on a single `/tmp/uniisland.sock` (#193).
    ...
    private static func remoteHookSource() -> String? {
        if let url = Bundle.appModule.url(forResource: "uniisland-remote-hook", withExtension: "py", subdirectory: "Resources"),
           let src = try? String(contentsOf: url) {
            return src
        }
        if let url = Bundle.appModule.url(forResource: "uniisland-remote-hook", withExtension: "py"),
           let src = try? String(contentsOf: url) {
            return src
        }
        return nil
    ...
    private static func remoteOpencodePluginSource() -> String? {
        if let url = Bundle.appModule.url(forResource: "uniisland-opencode-remote", withExtension: "js", subdirectory: "Resources"),
           let src = try? String(contentsOf: url) {
            return src
        }
        if let url = Bundle.appModule.url(forResource: "uniisland-opencode-remote", withExtension: "js"),
           let src = try? String(contentsOf: url) {
            return src
        }
        return nil
...
target = pathlib.Path.home() / ".uniisland" / "uniisland-remote-hook.py"
...
target = pathlib.Path.home() / ".uniisland" / "uniisland-opencode-remote.js"
target.parent.mkdir(parents=True, exist_ok=True)
target.write_bytes(base64.b64decode('''\(encoded)'''))
...
hook_path = home / ".uniisland" / "uniisland-remote-hook.py"
host_id = \(hostId)
host_name = \(hostName)
version = \(version)
...
def command_for(source):
    return f"UNIISLAND_SOCKET_PATH={socket_path} UNIISLAND_REMOTE_HOST_ID={json.dumps(host_id)} UNIISLAND_REMOTE_HOST_NAME={json.dumps(host_name)} UNIISLAND_SOURCE={source} python3 ~/.uniisland/uniisland-remote-hook.py"
...
def remove_our_hooks(hooks):
    for event in list(hooks.keys()):
        ...
        for entry in entries:
            ...
            if any("uniisland-remote-hook.py" in c for c in commands):
                continue
            next_entries.append(entry)
        if next_entries:
...
def install_opencode():
    ...
    plugin_path = home / ".uniisland" / "uniisland-opencode-remote.js"
    ...
    plugins = [
        p for p in plugins
        if not (isinstance(p, str) and ("vibe-island" in p or "uniisland" in p))
    ]
    ...
    if legacy_path.exists():
        ...
        if isinstance(legacy, dict) and isinstance(legacy.get("plugin"), list):
            cleaned = [p for p in legacy["plugin"] if not (isinstance(p, str) and ("vibe-island" in p or "uniisland" in p))]
    ...
    static func remoteOpencodePluginForInstall(source: String, host: RemoteHost, remoteSocketPath: String? = nil) -> String {
        ...
        return source
            .replacingOccurrences(
                of: #"const SOCKET_PATH = process.env.UNIISLAND_SOCKET_PATH || "/tmp/uniisland.sock";"#,
                with: #"const SOCKET_PATH = \#(jsonStringLiteral(socketPath));"#
            )
            .replacingOccurrences(
                of: #"const REMOTE_HOST_ID = process.env.UNIISLAND_REMOTE_HOST_ID || "";"#,
                with: #"const REMOTE_HOST_ID = \#(jsonStringLiteral(host.id));"#
            )
            .replacingOccurrences(
                of: #"const REMOTE_HOST_NAME = process.env.UNIISLAND_REMOTE_HOST_NAME || "";"#,
