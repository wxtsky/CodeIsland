...
enum NotchAnimation {
    ...
    static var open: Animation {
        .spring(response: NotchAnimationMetrics.openResponse, dampingFraction: 0.82)
    }
    /// 收起面板：临界阻尼，无过冲（防止 NotchPanelShape 底边露出刘海）
    static var close: Animation {
        .spring(response: NotchAnimationMetrics.closeResponse, dampingFraction: 1.0)
    }
    ...
    /// Contrast-edge hue crossfade when the featured session — and so the
    /// mascot — changes. Slow enough to register as a shift in light, not a blink.
    static let contrastEdgeTint = Animation.easeInOut(duration: 0.35)
