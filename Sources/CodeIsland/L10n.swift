...
final class L10n: ObservableObject {
    ...
    private static let en: [String: String] = [
        ...
        "show_claude_quota_desc": "Fetches your subscription limits (5-hour and weekly windows, as in /usage) from Anthropic using the Claude Code sign-in stored in Keychain. Makes network requests; macOS asks once for Keychain access. The token is only read, never refreshed.",
    ...
    private static let de: [String: String] = en.merging([
        ...
        "show_claude_quota_desc": "Ruft die Limits deines Abos (5-Stunden- und Wochenfenster, wie in /usage) über die im Schlüsselbund gespeicherte Claude-Code-Anmeldung von Anthropic ab. Nutzt das Netzwerk; macOS fragt einmal nach Schlüsselbund-Zugriff. Das Token wird nur gelesen, nie erneuert.",
    ...
    private static let zh: [String: String] = [
        ...
        "show_claude_quota_desc": "使用 Keychain 中保存的 Claude Code 登录信息，向 Anthropic 查询订阅额度（5 小时与周窗口，同 /usage）。会发起网络请求；macOS 会请求一次 Keychain 授权。只读取 token，不会刷新。",
    ...
    private static let zhHant: [String: String] = [
        ...
        "show_claude_quota_desc": "使用 Keychain 中儲存的 Claude Code 登入資訊，向 Anthropic 查詢訂閱額度（5 小時與週視窗，同 /usage）。會發起網路請求；macOS 會請求一次 Keychain 授權。僅讀取 token，不會重新整理。",
    ...
    private static let ja: [String: String] = [
        ...
        "show_claude_quota_desc": "キーチェーンに保存された Claude Code のサインインを使い、Anthropic からサブスクリプション上限（5時間・週ウィンドウ、/usage と同じ）を取得します。ネットワーク通信を行い、macOS がキーチェーンへのアクセスを一度確認します。トークンは読み取りのみで更新しません。",
    ...
    private static let ko: [String: String] = [
        ...
        "show_claude_quota_desc": "키체인에 저장된 Claude Code 로그인으로 Anthropic에서 구독 한도(5시간·주간 창, /usage와 동일)를 가져옵니다. 네트워크 요청을 보내며 macOS가 키체인 접근을 한 번 묻습니다. 토큰은 읽기만 하고 갱신하지 않습니다.",
    ...
    private static let tr: [String: String] = [
        ...
        "show_claude_quota_desc": "Anahtar Zinciri’ndeki Claude Code oturumunu kullanarak abonelik limitlerini (5 saatlik ve haftalık pencereler, /usage ile aynı) Anthropic’ten alır. Ağ isteği yapar; macOS bir kez Anahtar Zinciri erişimi sorar. Jeton yalnızca okunur, yenilenmez.",
